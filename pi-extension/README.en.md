...
- **Polling interval**: Tiered by task age (calculated from the server-side `created_at`) — polls once immediately after submission, then every 10 seconds for 0–2 minutes, every 20 seconds for 2–10 minutes, and every 30 seconds after 10 minutes
- **Rate limiting and backoff**: On 429 rate-limit responses, honors the server's `Retry-After` suggestion (clamped between 1 second and 5 minutes); network or server errors back off by consecutive failure count (60s → 120s → 240s → 300s cap), restoring the age-tiered interval after a success
