...
- **轮询间隔**：按任务年龄（从服务端 created_at 计算）分档——提交后立即轮询一次，0~2 分钟每 10 秒，2~10 分钟每 20 秒，10 分钟后每 30 秒
- **限流与退避**：遇到 429 限流响应时遵守服务端 Retry-After 建议（钳制在 1 秒至 5 分钟之间）；网络错误或服务器错误按连续失败次数退避（60 秒 → 120 秒 → 240 秒 → 300 秒封顶），成功后恢复年龄分档间隔
