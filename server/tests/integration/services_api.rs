...
async fn test_list_services_with_public_and_restricted() {
    ...
    assert_eq!(
        restricted_svc.unwrap()["access_type"].as_str(),
        Some("restricted")
    );
    ...
    assert_eq!(
        restricted_svc.unwrap()["has_permission"].as_bool(),
        Some(true)
    );
...
async fn test_admin_delete_service_success() {
    ...
    let (_, admin_api_key, _) = create_test_user(app.pool(), "admin", UserRole::Admin).await;
    let (service_id, _, _) =
        create_test_service(app.pool(), "delete_me", "Delete Me Service", true).await;
...
async fn test_admin_delete_service_with_tasks() {
    ...
    let (service_id, _, _) =
        create_test_service(app.pool(), "service_with_tasks", "Service With Tasks", true).await;
...
async fn test_get_public_service_usage_success() {
    ...
    let (_, client_api_key, _) = create_test_user(app.pool(), "clientuser", UserRole::Client).await;
    let (service_id, _, _) =
        create_test_service(app.pool(), "public_svc", "Public Service", true).await;
...
async fn test_get_restricted_service_usage_forbidden() {
    ...
    let (service_id, _, _) =
        create_test_service(app.pool(), "restricted_svc", "Restricted Service", false).await;
...
async fn test_get_restricted_service_usage_with_permission_success() {
    ...
    let (user_id, client_api_key, _) =
        create_test_user(app.pool(), "clientuser", UserRole::Client).await;
    let (service_id, _, _) =
        create_test_service(app.pool(), "restricted_svc2", "Restricted Service 2", false).await;
...
async fn test_admin_update_service_success() {
    ...
    let (_, admin_api_key, _) = create_test_user(app.pool(), "admin", UserRole::Admin).await;
    let (service_id, _, _) =
        create_test_service(app.pool(), "update_me", "Original Name", true).await;
...
async fn test_admin_update_service_partial() {
    ...
    let (_, admin_api_key, _) = create_test_user(app.pool(), "admin", UserRole::Admin).await;
    let (service_id, _, _) =
        create_test_service(app.pool(), "partial_update", "Original Name", true).await;
...
async fn test_admin_update_service_empty_body() {
    ...
    let (_, admin_api_key, _) = create_test_user(app.pool(), "admin", UserRole::Admin).await;
    let (service_id, _, _) =
        create_test_service(app.pool(), "empty_update", "Empty Update", true).await;
