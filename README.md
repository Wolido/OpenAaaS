...
**OpenAaaS 是一个 Agent 编排网络（Agent Orchestration Network）：数据驻留在产生它的原地，Agent 能力通过网络流动到数据身边。**

AI 的瓶颈已从模型能力转向科研能力的可达性，而"数据被迫迁移"是比模型更硬的约束。每个实验室都沉淀了独特的数据、算法与流程，但它们分散在孤岛中，无法被发现与调用。OpenAaaS 构建一个 Agent 编排网络，让任意 Agent 都能发现、委派并组合全球科研节点上的其他 Agent——数据原地处理，Agent 能力在网络中流动。
...
传统思路是：Agent 通过网络调用远程脚本或 API，远程节点被动执行一段预设代码，再把结果返回。

OpenAaaS 反其道而行之：**你的 Agent 调用的不是脚本，而是另一个完整的 Agent 实例。**

在这个网络中，存在清晰的**主从层次**：

- **主 Agent（客户端 Agent）**：理解你的任务，在全网发现合适的远程节点，将任务**委派**给子 Agent，并整合回传的结果。
- **子 Agent（远程 Agent）**：运行在数据本地的 Docker 容器中，被委派任务后像一位"数字同事"一样**自主决策、自主执行**，完成后将结果回传给主 Agent。

每个 Agent Core 节点通过 Docker 容器运行一个**拥有完整工具链的 Agent**——pi mono、Kimi Cli、Codex、Claude Code、Open Code 或自研 Agent。这个子 Agent 拥有 read、write、bash、grep、subagent、todo 等自主工具。主 Agent 不是去"调用"一个远程函数，而是**发现**一个合适的子 Agent，**委派**任务给它；子 Agent 在数据旁边自主工作，结果回传后主 Agent 将其整合到自己的工作流中。这是一种**单向的委派关系**，不是对等双向的调用。

这意味着 OpenAaaS 本质上是一个 **Agent 编排网络**——主 Agent 通过网络发现、委派并组合多个子 Agent，而不是调用脚本或服务。
...
A2A 是优秀的通用协议，但科研网络里容易"断线"——防火墙掐长连接，NAT 静默丢包，IT 不给开端口。OpenAaaS 每隔几秒主动问一次，不用一直占着连接，中间设备不会无声切断。节点只发出站请求，不需要公网 IP，不用求 IT。单二进制复制即用。OpenAaaS 不是"科研专属"，而是**特殊网络环境下最稳妥的智能体互联方案**。
...
| Agent 编排 | 容器内运行完整 Agent 实例（非脚本），拥有 read/write/bash/grep/subagent 等自主工具链 | 远程 Agent 自主决策、自主执行复杂任务，客户端只需描述目标 |
